...
exploration. Perhaps the most straigth-forward thing to do is count
...
The data has been choosen so that each year has the same number of
...
This shows some interesting aspects of the data. Males are on aveage
...
10.9 years, wheras the average Male age changes by about 12.3 years;
perhaps this reflects different life expectancies of males and females.

Also interesting is that the average weight changes, by about 8.2 kg
(18 lbs) for Female respondenents, 7.6 kg (16.7 lbs) for Males. This
...
The bar in the figure is the median weigth, the box represents upper
and lower quartiles of the data, the whiskers extend to 1.5 times the
interquartile range. Points outside the whiskers represent potential
...
`geom_point()` to generate a scatterplot showing this relationship
...
Or we might use a statistic like the Wilcoxon test that make fewer
...
Weight between 1990 and 2010 is highly signficant.

### Linear regression

The 'BRFSS Male Subset' figure above shows linear relations between
the square root of Weight and Height. How would we calculate this
regression, and assess its significance?
...
result. We focus on Male 2010 data.
