...
a bioinformatics core. _R_ can be used to summarize biological sample
...
```{r survfit}
fit <- survfit(Surv(time, status) ~ 1, lung)
fit
```
...
```{r survfit-sex}
fit_sex <- survfit(Surv(time, status) ~ sex, lung)
fit_sex
